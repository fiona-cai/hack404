...
export async function GET(request: NextRequest) {
    ...
    const nearbyCoordinates = coordinates.filter(coord => coord.user_id !== userId);
    ...
    const userCoordinate = coordinates.find(coord => coord.user_id === userId);
    if (!userCoordinate) {
    ...
    const nearby = nearbyCoordinates.filter(coord => {
        const distance = Math.sqrt(
            Math.pow(coord.latitude - userCoordinate.latitude, 2) +
            Math.pow(coord.longitude - userCoordinate.longitude, 2)
        );
