...
export default function GestureCatchCard({
  ...
  const { acceleration, requestPermission } = useDeviceMotion();
  ...
  return (
    <div className={`relative ${className}`}>
      <motion.div
        className="absolute -top-16 left-1/2 transform -translate-x-1/2 text-white text-sm font-medium bg-black/50 px-4 py-2 rounded-full"
      ...
      <motion.div
        ref={cardRef}
        className={`
          relative w-80 h-48 bg-gradient-to-br from-blue-500 to-purple-600 
          rounded-2xl shadow-xl cursor-grab active:cursor-grabbing
      ...
      >
        <div className="absolute inset-0 rounded-2xl overflow-hidden">
          <div className="absolute top-0 left-0 right-0 h-1/2 bg-gradient-to-b from-red-400 to-red-500" />
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center text-white z-10">
          <div className="relative mb-3">
            <Image
              src={`/images/${targetUser.avatar}.png`}
              alt={targetUser.name}
              width={64}
              height={64}
          ...
          <h3 className="text-lg font-bold text-shadow">{targetUser.name}</h3>
      ...
      <div className="flex justify-center gap-4 mt-6">
        <motion.button
          className="px-6 py-3 bg-green-500 hover:bg-green-600 text-white font-bold rounded-full shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
          onClick={handleCatch}
        ...
        <motion.button
          className="px-4 py-3 bg-blue-500 hover:bg-blue-600 text-white font-bold rounded-full shadow-lg text-sm"
