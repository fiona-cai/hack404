    ...
    try {
      ...
      const nearbyLayers = nearbyUsers.map(user => {
        ...
        return new ScenegraphLayer<User>({
          ...
          onClick: () => {
            console.log(`Clicked on user ${user.user_id}:`, user);
            handleShowGestureCatch(user);
          }
        });
      }).filter(Boolean);
    ...
    <div style={{ position: 'relative', width: '100vw', height: '100vh' }}>
      ...
      <DeckGL
        initialViewState={{
          ...
          zoom: 20,
          bearing: 0, // Use device compass heading
          pitch: 60
        }}
        // viewState={{
        //   longitude: coords.longitude,
      ...
      <div style={{ position: 'absolute', top: 20, left: 20, zIndex: 0, background: 'rgba(0,0,0,0.7)', borderRadius: 16, padding: 16, color: '#fff', minWidth: 220 }}>
      ...
      {showGestureCatch && (
        <CatchUserModal
          ...
          setShowCatchAnimation={showGestureCatch.setShowCatchAnimation}
        />
      )}
    </div>
  );
